...
const ContactUs = () => {
  ...
  const contactFormSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if ((resp as any)?.data?.success || (resp as any)?.data?.message) {
        toast.success(
          (resp as any)?.data?.message || "Message sent successfully!"
        );
        setContactFormData({ ...initialState });
      } else if ((resp as any)?.error) {
        toast.error(
          (resp as any)?.error?.data?.message || "Failed to send message."
        );
      }
    } catch (err: any) {
        ...
        <div className="w-[95%] md:w-[50%] flex flex-col justify-start items-center gap-20 mb-20 md:mb-0 md:items-start">
          ...
          <div className="flex justify-start gap-5">
            ...
            <div>
              ...
              <p className="">tanwir432003@gmail.com</p>
              <p>tanwir432003@gmail.com</p>
            </div>
          </div>
        </div>
      </div>
